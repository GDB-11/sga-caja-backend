package com.cibertec.sga.currency;
...
class CurrencyIntegrationTest extends AbstractIntegrationTest {
    ...
    @Test
    void listReturnsSeededCurrencies() throws Exception {
        mockMvc.perform(get("/api/currencies").header("Authorization", authHeader))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.length()").value(2));
    }

    @Test
    void getByUuidReturnsMatchingCurrency() throws Exception {
        MvcResult list = mockMvc.perform(get("/api/currencies").header("Authorization", authHeader)).andReturn();
        String uuid = objectMapper.readTree(list.getResponse().getContentAsString()).get(0).get("uuid").asText();

        mockMvc.perform(get("/api/currencies/{uuid}", uuid).header("Authorization", authHeader))
            .andExpect(status().isOk())
    ...
    void getByUnknownUuidReturnsNotFound() throws Exception {
        mockMvc.perform(get("/api/currencies/{uuid}", UUID.randomUUID()).header("Authorization", authHeader))
